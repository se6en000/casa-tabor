...
import { formatDueByForAiPrompt } from '../src/utils/eventTime.ts'
...
// Fix: format the due date into an explicit, unambiguous Eastern-time phrase
// before it's ever embedded in the AI prompt.

test('formatDueByForAiPrompt spells out an unambiguous Eastern-time phrase, not a raw UTC ISO string', () => {
  // 16:00 UTC on Aug 6 2026 is noon Eastern (EDT, UTC-4) in summer.
  const result = formatDueByForAiPrompt('2026-08-06T16:00:00.000Z')
  assert.match(result, /Eastern Time/)
...
test('Prep item detail panel and Action Hub no longer hand the AI a raw ISO due_by timestamp', () => {
  for (const source of [prepPanel, actionHub]) {
    assert.match(source, /formatDueByForAiPrompt/)
