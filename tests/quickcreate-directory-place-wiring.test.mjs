...
test('QuickCreateSheet resolves the place selection to an existing or newly-created saved_places row before insert', () => {
  ...
  const insertIndex = source.indexOf('const eventPayload = {')
  assert.ok(insertIndex >= 0, 'quick-create must build its RPC payload from eventPayload')
  assert.match(source.slice(insertIndex), /rpc\('upsert_event_bundle', \{ p_payload: eventPayload \}\)/)
