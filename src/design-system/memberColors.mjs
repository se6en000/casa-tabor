...
const MEMBER_COLOR_NAME_BY_HEX = new Map(
...
export const getDisplayMemberColor = (hex) =>
  hex && typeof hex === 'string' && hex.trim().length > 0 ? hex : FALLBACK_PROFILE_COLOR

export const getMemberColorName = (hex) =>
  MEMBER_COLOR_NAME_BY_HEX.get(hex) ?? hex

export function getMemberRoleLabel(member) {
  if (!member) return 'Member'
  const role = member.role ? String(member.role).toLowerCase() : ''
  const canDrive = Boolean(member.can_drive)
  if (role === 'parent') {
    return canDrive ? 'Parent · Driver' : 'Parent'
  }
  if (role === 'caregiver' || role === 'care_giver') {
    return canDrive ? 'Caregiver · Driver' : 'Caregiver'
  }
  if (role === 'child') {
    return canDrive ? 'Child · Driver' : 'Child'
  }
  const base = member.role ? String(member.role).charAt(0).toUpperCase() + String(member.role).slice(1) : 'Member'
  return canDrive ? `${base} · Driver` : base
}
