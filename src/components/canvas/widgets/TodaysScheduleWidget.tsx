...
import { Calendar, ChevronDown, ChevronUp, ChevronRight, CheckCircle2, Check } from 'lucide-react'
            ...
            {pastEvents.length > 0 && (
              <div className="mb-2 px-1">
                <Button
                  ...
                  className="min-h-[32px] h-8 py-0.5 px-2.5 rounded-lg bg-casa-surface-subtle/80 hover:bg-casa-surface-subtle text-caption text-casa-muted hover:text-casa-navy border border-casa-border/30 transition-colors"
                >
                  <span className="inline-flex items-center gap-1.5 font-normal">
                    <CheckCircle2 size={13} className="text-emerald-600/80 shrink-0" />
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          ...
                          <span className="text-body-sm truncate line-through text-casa-muted group-hover:text-casa-navy transition-colors">
                            {evt.title}
                          </span>
                          {evt.location_name && (
              ...
              {upcomingAppointments.map((evt) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      'relative flex items-center justify-between gap-4 py-3.5 pl-6 pr-2 -ml-7 rounded-xl transition-all duration-150 cursor-pointer group active:scale-[0.99] min-h-[56px]',
                      isNow
                        ? 'bg-emerald-500/8 hover:bg-emerald-500/12 border border-emerald-500/30'
                        : 'hover:bg-casa-surface-subtle/60'
                    )}
                  >
                    <span
                      className={cn(
                    ...
                    <div className="flex items-center gap-4 min-w-0 flex-1">
                      {evt.all_day ? (
                        ...
                        <span className="font-mono text-body-sm font-bold text-casa-navy shrink-0 tabular-nums w-16">
                          {format(parseISO(evt.start_time), 'h:mm a')}
                        </span>
                      )}
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {isNow && (
                            <span className="inline-flex items-center gap-1 text-3xs font-bold uppercase tracking-wider text-emerald-800 bg-emerald-500/20 px-1.5 py-0.5 rounded-full shrink-0">
