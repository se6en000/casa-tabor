...
- [~] **P3.10 — Getting around the wall, and adding by touch** — added 2026-09-25; design board section 04 (04a–04d) approved by Jake ("the navigation looks great, approved to build").
  ...
  - Claimed: Claude (Opus 5.5), 2026-09-25. Part 1 (getting around) shipped in `281dfe2b` (this entry follows in the next commit; the plan edit failed in that one): `selectPosture` (2-hour lead, someone out), `tomorrowLine` (posture.ts), `weekDays` "Tomorrow" and "N to do", `fitPackingColumns` (packing.ts), `WallEvening` rebuilt as the day-ahead face (heading, dark/light, week strip, Back), `WallTomorrowNote`, `WallView` (the day on show, wake on Calm, menu previews), `WallMenu` "Preview a face", week-wide checklist fetch. Tests: `wall-posture` (12, incl. the new rules and the tomorrow line), `wall-week` (5), `wall-packing` (9); Playwright: Calm wakes and menu previews, the week strip into the day-ahead face and back, the tomorrow line opening tomorrow (`tomorrow-note.png`, `week-saturday.png`), plus every earlier flow. A tap on a person on Calm used to be swallowed by the wake (found by the test); now only an unhandled tap wakes. **Remaining:** part 2, adding by touch (04d); kiosk check.
