...
// resolveDefaultCalendarCreate (a regex fast-path that skipped the LLM planner for
// simple "book an appointment at TIME" requests) was retired: it kept accumulating
// narrow regex bugs (see git history) that the already-100%-rolled-out ai-agent-write
// planner doesn't have, since that planner already has an equivalent safety net
// (classifyCalendarTemporalEvidence, in ai-assistant/index.ts) for the same
// no-date-evidence case. All calendar.create requests now go through the planner.
