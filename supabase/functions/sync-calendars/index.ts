...
async function upsertEvent(sb: SupabaseClient, sourceMemberId: string, ev: Record<string, unknown>) {
  ...
  if (existing) {
    ...
    // New event — before inserting, check if a trip leg already exists at this time for this member.
    // This prevents Google Calendar sync from creating a duplicate when there's already a
    // trip leg event (flight/hotel) that was generated by scan-travel-emails and pushed to Google.
    const { data: existingLeg } = await sb.from('events')
      .select('id')
      .eq('source_member_id', sourceMemberId)
      .eq('start_time', startTime)
      .not('leg_type', 'is', null)
      .maybeSingle()
    if (existingLeg) return  // a richer trip leg already exists — skip
