-- Real due dates for the To-Do <-> Reminders sync (2026-09-17): the Mac-side
-- script was confirmed (via its actual EKReminder loop, not a paraphrase) to
-- never read dueDateComponents at all -- every synced to-do was silently
-- defaulting to "end of today" even when the user set a real date/time on
-- the phone. This adds an optional due date to the ingest RPC: when present
-- and time-bearing, use it exactly; when present but date-only (EventKit
-- reminders can have a date with no time), default to 9am household-local
-- on that date; when genuinely absent, keep the existing end-of-today
-- fallback from 20260917130000_todo_reminder_sync.sql.
drop function if exists public.upsert_todo_reminder_from_ios(text, text, boolean, boolean, timestamptz);
...
revoke all on function public.upsert_todo_reminder_from_ios(text, text, boolean, boolean, timestamptz, timestamptz, boolean)
  from public, anon, authenticated;
grant execute on function public.upsert_todo_reminder_from_ios(text, text, boolean, boolean, timestamptz, timestamptz, boolean)
