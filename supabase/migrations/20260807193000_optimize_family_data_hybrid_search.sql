...
as $$
  ...
  semantic_candidates as (
    ...
    limit greatest(40, least(match_count * 4, 200))
  ),
  lexical_candidates as (
    select chunk.id
    from public.family_data_chunks as chunk
    cross join lexical_query
    where chunk.search_vector @@ lexical_query.value
  ...
  entity_candidates as (
    ...
    where cardinality(query_entities) > 0
      and exists (
        select 1
  ...
  scored as (
    select
      ...
      ts_rank_cd(chunk.search_vector, lexical_query.value)::double precision as lexical_score,
      case
    ...
    from candidate_ids
    join public.family_data_chunks as chunk on chunk.id = candidate_ids.id
    join public.family_data_documents as document on document.id = chunk.document_id
    cross join lexical_query
