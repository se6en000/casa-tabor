-- System health monitoring + AI circuit breaker auto-trip.
--
-- settings.ai_circuit_breaker already existed (written by the Cost & Usage page) but
-- nothing ever read it -- the "Pause AI" switch was dead. Edge functions now enforce
-- it (_shared/provider-call-ledger.mjs); this migration adds the other half:
--   * evaluate_system_health(): every 15 min, checks AI spend, calendar-feed latency,
--     background edge-function errors, cron failures and client error bursts; opens /
--     resolves rows in system_alerts; auto-trips the breaker on runaway AI usage.
--   * get_system_health_summary() / get_active_system_alerts(): read paths for the
--     System Health settings page and the home-screen alert banner.
--   * set_ai_circuit_breaker(): pause/resume that merges into the existing config
--     (the page used to overwrite the whole value) and stamps resumed_at.
--
-- Thresholds come from real 30-day usage (2026-09-23): worst hour $0.17 / 460k tokens /
-- 188 calls, worst day $0.71. Defaults sit ~3x above those; override any of them via
-- settings.ai_circuit_breaker (hourly_cost_cap_usd, daily_cost_cap_usd,
-- hourly_token_cap, hourly_call_cap, auto_trip_enabled).

create table if not exists public.system_alerts (
  id uuid primary key default gen_random_uuid(),
  alert_key text not null,
  category text not null
    check (category in ('ai_spend', 'latency', 'background_calls', 'cron', 'client_errors')),
...
create unique index if not exists system_alerts_open_key_uidx
  on public.system_alerts (alert_key) where resolved_at is null;
create index if not exists system_alerts_opened_at_idx
  on public.system_alerts (opened_at desc);

alter table public.system_alerts enable row level security;
revoke all on public.system_alerts from anon, authenticated;

create table if not exists public.query_latency_snapshots (
...
create index if not exists query_latency_snapshots_label_idx
  on public.query_latency_snapshots (query_label, captured_at desc);

alter table public.query_latency_snapshots enable row level security;
revoke all on public.query_latency_snapshots from anon, authenticated;

-- ── Helpers ──────────────────────────────────────────────────────────────────

create or replace function public.ai_circuit_breaker_config()
returns jsonb
language sql
stable
security definer
...
as $$
  select jsonb_build_object(
    ...
    'hourly_token_cap', 1500000,
    'hourly_call_cap', 600,
    'auto_trip_enabled', true
  ) || coalesce((select value from public.settings where key = 'ai_circuit_breaker'), '{}'::jsonb)
$$;

-- Usage since each window start, excluding calls the breaker itself refused.
create or replace function public.ai_spend_window(p_hour_start timestamptz, p_day_start timestamptz)
returns table (
  hour_cost_usd numeric,
  hour_tokens bigint,
  hour_calls bigint,
  user_hour_cost_usd numeric,
  user_hour_tokens bigint,
  user_hour_calls bigint,
  day_cost_usd numeric
)
language sql
stable
security definer
...
as $$
  with priced as (
    select
      c.traffic_class,
      c.occurred_at,
      (c.input_tokens + c.output_tokens + c.thought_tokens)::bigint as tokens,
      case when c.status = 'success' then (
        c.input_tokens * coalesce(p.input_per_million_usd, 0)
        + c.output_tokens * coalesce(p.output_per_million_usd, 0)
        + c.thought_tokens * coalesce(p.thought_per_million_usd, p.output_per_million_usd, 0)
      ) / 1000000::numeric else 0::numeric end as cost
    ...
    where c.occurred_at >= least(p_hour_start, p_day_start)
      and c.status <> 'cancelled'
  )
  select
    coalesce(sum(cost) filter (where occurred_at >= p_hour_start), 0),
    coalesce(sum(tokens) filter (where occurred_at >= p_hour_start), 0)::bigint,
    count(*) filter (where occurred_at >= p_hour_start),
    coalesce(sum(cost) filter (where occurred_at >= p_hour_start and traffic_class = 'user'), 0),
    coalesce(sum(tokens) filter (where occurred_at >= p_hour_start and traffic_class = 'user'), 0)::bigint,
    count(*) filter (where occurred_at >= p_hour_start and traffic_class = 'user'),
    coalesce(sum(cost) filter (where occurred_at >= p_day_start), 0)
  from priced
$$;

-- Opens an alert, or refreshes the already-open one with the same key.
-- Returns true only when a new alert was opened.
create or replace function public.upsert_system_alert(
  p_key text,
  p_category text,
  p_severity text,
  p_title text,
...
begin
  update public.system_alerts
     set severity = p_severity, title = p_title, detail = p_detail,
         metrics = p_metrics, last_seen_at = now()
   where alert_key = p_key and resolved_at is null
  returning id into v_id;
  if v_id is not null then
    return false;
  end if;
  insert into public.system_alerts (alert_key, category, severity, title, detail, metrics)
  values (p_key, p_category, p_severity, p_title, p_detail, p_metrics);
  return true;
end;
$$;
...
create or replace function public.set_ai_circuit_breaker(
  p_paused boolean,
  p_scope text default 'background',
...
declare
  ...
  v_next jsonb;
begin
  if p_paused and p_scope not in ('background', 'all') then
    raise exception 'invalid pause scope: %', p_scope;
  end if;

  select value into v_current from public.settings where key = 'ai_circuit_breaker';

  v_next := coalesce(v_current, '{}'::jsonb) || case
    when p_paused then jsonb_build_object(
      'paused', true, 'pause_scope', p_scope, 'pause_until', p_pause_until,
      'tripped_by', 'manual', 'tripped_at', now(), 'trip_reason', null)
    else jsonb_build_object(
      'paused', false, 'pause_scope', 'none', 'pause_until', null, 'resumed_at', now())
  end;

  insert into public.settings (key, value, updated_at)
  values ('ai_circuit_breaker', v_next, now())
  on conflict (key) do update set value = excluded.value, updated_at = excluded.updated_at;

  -- Resuming is an explicit "I've looked at it"; the evaluator also only counts usage
  -- after resumed_at, so the spike that tripped it can't immediately re-trip it.
  if not p_paused then
    update public.system_alerts
       set resolved_at = now()
     where resolved_at is null and alert_key = 'ai_spend:runaway';
  end if;

  return public.ai_circuit_breaker_config();
end;
$$;

-- ── Read paths ─────────────────────────────────────────────────────────────────

-- Lightweight: polled every minute by the home-screen banner on every device.
create or replace function public.get_active_system_alerts()
returns jsonb
language sql
stable
security definer
...
as $$
  select jsonb_build_object(
    'breaker', public.ai_circuit_breaker_config(),
    'critical_alerts', coalesce((
      select jsonb_agg(jsonb_build_object(
        'id', a.id, 'alert_key', a.alert_key, 'category', a.category,
        'title', a.title, 'detail', a.detail, 'opened_at', a.opened_at
...
declare
  ...
  v_spend record;
begin
  select * into v_spend from public.ai_spend_window(
    greatest(now() - interval '1 hour', v_since_resume),
    greatest(date_trunc('day', now() at time zone v_tz) at time zone v_tz, v_since_resume)
  );
  ...
  return jsonb_build_object(
    ...
    'last_check', (select value from public.settings where key = 'system_health_last_check'),
    'open_alerts', coalesce((
      select jsonb_agg(to_jsonb(a) - 'metrics' order by
        case a.severity when 'critical' then 0 else 1 end, a.opened_at desc)
      from public.system_alerts a
      where a.resolved_at is null
    ), '[]'::jsonb),
    'recent_alerts', coalesce((
      select jsonb_agg(to_jsonb(r) - 'metrics' order by r.opened_at desc)
      from (
        select * from public.system_alerts
        where resolved_at is not null and opened_at >= now() - interval '7 days'
        order by opened_at desc
        limit 20
      ) r
    ), '[]'::jsonb),
    'client_errors', jsonb_build_object(
      'last_24h', (select count(*) from public.client_error_log where occurred_at >= now() - interval '24 hours'),
      'last_7d', (select count(*) from public.client_error_log where occurred_at >= now() - interval '7 days'),
      -- Grouped summary only: no stacks, no user agents.
      'top', coalesce((
        ...
        from (
          select
            ...
            (array_agg(split_part(split_part(e.url, '?', 1), '#', 1) order by e.occurred_at desc))[1] as last_url
          from public.client_error_log e
          where e.occurred_at >= now() - interval '7 days'
          group by left(e.message, 200), e.source
...
revoke execute on function public.ai_spend_window(timestamptz, timestamptz) from public, anon, authenticated;
revoke execute on function public.upsert_system_alert(text, text, text, text, text, jsonb) from public, anon, authenticated;
revoke execute on function public.evaluate_system_health() from public, anon, authenticated;
revoke execute on function public.ai_circuit_breaker_config() from public, anon, authenticated;

-- settings (where the breaker lives) is already fully readable/writable by the app's
-- anon role, so exposing pause/resume and the summaries adds no new access.
grant execute on function public.set_ai_circuit_breaker(boolean, text, timestamptz) to anon, authenticated;
grant execute on function public.get_active_system_alerts() to anon, authenticated;
grant execute on function public.get_system_health_summary() to anon, authenticated;

-- Pure SQL, no net.http_post. Staggered onto minutes no other job uses
-- (see 20260923001500_stagger_colliding_cron_schedules.sql).
select cron.unschedule('evaluate-system-health')
where exists (select 1 from cron.job where jobname = 'evaluate-system-health');

select cron.schedule('evaluate-system-health', '13,28,43,58 * * * *', $$select public.evaluate_system_health()$$);
