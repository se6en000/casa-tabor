-- Bug: discover_directory_candidates() could surface the exact same
-- real-world entity twice — once as a "place" suggestion and once as a
-- "contact" suggestion — when an event's location_name and its
-- event_enrichments.contact_name were identical (e.g. an event whose
-- "contact" is really just the venue's front-desk phone number, like a
-- golf club, restaurant, or medical office main line). The place-scan and
-- contact-scan CTEs never cross-checked each other, so both inserted a
-- row for the same name from the same event, producing two separate
-- "Auto-detected" Needs You review cards with identical text.
--
-- Fix: candidate_contacts now excludes any contact_name that exactly
-- matches (case-insensitive, trimmed) the same event's own location_name
-- — that pattern means the "contact" IS the venue, which is already
-- covered by the place suggestion. Everything else (family-member
-- exclusion, phone/category aggregation) is unchanged.
...
begin
  ...
  with candidate_contacts as (
    ...
    where ee.contact_name is not null
      ...
      -- Skip "contacts" that are really just the event's own venue (same
      -- name as location_name) — that entity is already surfaced as a
      -- place suggestion above, and duplicating it as a contact too was
      -- confusing two identical review cards for one real place.
      and (
        e.location_name is null
        or lower(trim(ee.contact_name)) <> lower(trim(e.location_name))
      )
  ),
