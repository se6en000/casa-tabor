-- Fixes a foreign-key failure in the connections candidate query added by
-- 20260805224500: household_graph_nodes rows can go stale between graph
-- rebuilds (e.g. a place was merged/deleted after the node was written), so
-- pn.ref_id is no longer guaranteed to exist in saved_places. Join directly
-- against saved_places to only consider live place ids.
