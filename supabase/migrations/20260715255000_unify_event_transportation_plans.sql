alter table public.event_plan_overrides
  add column if not exists location_projection_blocked boolean not null default false;

comment on column public.event_plan_overrides.location_projection_blocked is
  'True only for an automatically resolved address that must not reach Google until human confirmation.';
