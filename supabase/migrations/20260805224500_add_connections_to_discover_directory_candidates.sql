-- Extends discover_directory_candidates() with a fourth candidate type:
-- contact <-> place "Connections" (contact_place_relationships). Same
-- additive-only, idempotent contract as the other three: only suggested when
-- a contact's event_enrichments.contact_name and the event's graph-linked
-- place co-occur, and only when no relationship row (confirmed or not)
-- already exists for that (contact, place) pair.
